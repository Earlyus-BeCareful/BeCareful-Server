...
public enum ErrorCode {
    ...
    SMS_SEND_FAILED(BAD_REQUEST, "SMS 전송 중 문제가 발생했습니다."),
    SMS_PHONE_NUMBER_AUTH_NOT_EXISTS(BAD_REQUEST, "해당 전화번호로 요청된 인증이 없습니다."),
    SMS_AUTHENTICATION_FAILED(CONFLICT, "인증번호가 일치하지 않습니다."),

    FAILED_TO_CREATE_IMAGE_FILE_NAME(INTERNAL_SERVER_ERROR, "프로필 이미지를 업로드하는 중 문제가 발생했습니다."),

    CAREGIVER_ALREADY_EXISTS(CONFLICT, "이미 가입된 전화번호 입니다."),
    CAREGIVER_NOT_EXISTS(NOT_FOUND, "해당하는 요양보호사가 없습니다."),
    CAREGIVER_REQUIRED_AGREEMENT(BAD_REQUEST, "필수 동의 항목에 동의하지 않았습니다."),
    CAREGIVER_WORK_APPLICATION_NOT_EXISTS(NOT_FOUND, "일자리 신청서가 존재하지 않습니다."),
    CAREGIVER_CAREER_NOT_EXISTS(NOT_FOUND, "요양보호사 경력사항이 존재하지 않습니다.");
    ...
    private ErrorCode(HttpStatus statusCode, String message) {
