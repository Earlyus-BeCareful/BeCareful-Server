...
public record ChatRoomActiveStatusUpdatedChatResponse(ChatReceiveType chatType, ChatRoomActiveStatus status)
        implements ChatResponse {
    public static ChatRoomActiveStatusUpdatedChatResponse of(ChatRoomActiveStatus status) {
        return new ChatRoomActiveStatusUpdatedChatResponse(ChatReceiveType.CHATROOM_ACTIVE_STATUS_UPDATED, status);
    }
}
